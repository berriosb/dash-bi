# Impeccable Adoption — Plan archivado

> **Status:** Ejecutado el 2026-07-24. Movido de `.hermes/plans/` a `docs/design/` para preservar el conocimiento una vez consumido el plan.
>
> Ver [`docs/agent-skills.md`](../agent-skills.md) para el estado actual del setup de skills y los cambios de vendorización posteriores.
...
**Architecture:** Impeccable skills install into provider-native folders (`.opencode/skills/`, `.claude/skills/`, `.agents/skills/`). Only dash-bi-authored skills are tracked in git; all third-party skill files and the Impeccable local config (`.impeccable/`) are gitignored and regenerated by contributors from `skills-lock.json` or installed via the Impeccable CLI. See [`docs/agent-skills.md`](../agent-skills.md) for the canonical split.
...
7. ~~Leave changes uncommitted for Bastian's review; do not push.~~ Ejecutado y commiteado en `a10d7d9` (Sprint 1 close) y `ab93aa9` (security-audit skill). Este archivo fue movido de `.hermes/plans/` a `docs/design/` el 2026-07-27.

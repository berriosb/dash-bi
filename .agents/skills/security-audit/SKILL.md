...
3. **Walk the 10 controls** against the target. Use `grep -n` to find patterns; read the surrounding code to confirm context. Never flag without verifying the pattern is actually a violation (e.g., `db.select()` inside a `withOrgContext` callback is fine).
